...
void Pi_BSR(BigFloat& P, BigFloat& Q, BigFloat& R, uint32_t a, uint32_t b, size_t p, int& height, int tds = 1) {
	...
	if (b - a < 1000 || tds == 1) {
		int h1, h2;
		Pi_BSR(P0, Q0, R0, a, m, p, h1, tds);
		Pi_BSR(P1, Q1, R1, m, b, p, h2, tds);
		height = max(h1, h2) + 1;
	}
	else {
		int tds0 = tds / 2;
		int tds1 = tds - tds0;
		int h1, h2;
		std::thread x([&]() {Pi_BSR(P0, Q0, R0, a, m, p, h1, tds0); });
		std::thread y([&]() {Pi_BSR(P1, Q1, R1, m, b, p, h2, tds1); });
		height = max(h1, h2) + 1;
		x.join();
		y.join();
	}
...
void Pi(size_t decimal_digits, size_t threads) {
	...
	time_init = time0;
	int h;
	Pi_BSR(P, Q, R, 0, (uint32_t)terms, p, h, (int)threads);
